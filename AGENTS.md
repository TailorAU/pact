...
**v2.0 shipped 14 May 2026** (tag `v2.0.0`; mirror sync via PR [TailorAU/tailor-app#1616](https://github.com/TailorAU/tailor-app/pull/1616)). **v2.0.1 patch released 15 May 2026** (tag `v2.0.1`; mirror sync via PR TBD — see release notes). v2.0.1 lands the cold-eye-audit fixes: §17.4 `did:web` security note, §17.9 Authorization-Required tier rewritten with concrete checks, §17.10 GDPR softening, §17.11 delegation deferred to v2.1, §15.1 `retentionPolicy` required, §23.4 quorum-enrollment-implementation-defined note, schema verifier-binding `if/then`, runner verdict honesty (`✓ verified-structural`), HTTP-coverage CI warning. See `CHANGELOG.md` v2.0.1 for the full list. The PACT v2 roadmap (`docs/v2-plan.yaml`) is mostly done; decisions D1–D6 are recorded in `docs/v2-prep/d1-d6-decisions.yaml`.
