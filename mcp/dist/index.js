...
function createServer() {
    ...
    server.tool("source_claim_price_assignment", "Claim an open price_observation_mining assignment for a (item_key, retailer) pair. Daily cron opens assignments for stale (>24h) pairs across the 5 hardware retailers (bunnings, mitre-10, reece, beaumont-tiles, tradelink). Use this then call source_submit_price_observation with the same assignmentId. Requires SOURCE_AGENT_KEY.", {
        ...
        try {
            ...
            if (retailerSlug)
                payload.retailerSlug = retailerSlug;
            return jsonResult(await postAgent("/api/work/claim", {
    ...
    }, async ({ assignmentId, itemKey, retailerSlug, productName, productUrl, ean, priceCents, unitPriceCents, unitPriceUnit, inStock, }) => {
        try {
            return jsonResult(await postAgent("/api/work/submit", {
                assignmentId,
                submission: {
                    itemKey,
                    retailerSlug,
