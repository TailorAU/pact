#!/usr/bin/env python3
"""
Tailor <> PACT Legislation API Integration
===============================================

Demonstrates how Tailor's compliance pipeline can replace website scraping
and LLM-based clause extraction with a single PACT API call.
...
New pipeline (PACT Legislation API):
...
# ---------------------------------------------------------------------------
# PACT Legislation API client
# ---------------------------------------------------------------------------

class PactApiError(Exception):
    """Raised when the PACT API returns an error response."""
...
class PactLegislationClient:
    """Client for the PACT Legislation API (Axiom tier).
    ...
    def __init__(
        ...
        Args:
            ...
            base_url: PACT base URL. Defaults to production.
...
class PreParsedLegislationExtractor(IExtractionStrategy):
    """Extracts sections via PACT's pre-parsed legislation API.
    ...
    def extract(self, legislation_url: str) -> list[ExtractedSection]:
        """Fetch pre-parsed sections from PACT.
...
class ExtractionStrategyResolver:
    ...
    If PACT has the document pre-parsed, uses PreParsedLegislationExtractor
    ...
    def resolve(self, legislation_url: str) -> IExtractionStrategy:
        ...
        if doc_id is not None:
            logger.info(
                "PACT has document '%s' -> using PreParsedLegislationExtractor",
                doc_id,
            )
            return PreParsedLegislationExtractor(self._client, doc_id)

        logger.info(
            "Document not in PACT -> falling back to ClaudeTreeExtractor"
        )
        return ClaudeTreeExtractor()

    def _lookup_pact_doc_id(self, url: str) -> Optional[str]:
        """Look up a legislation URL in PACT.
...
def generate_demo_api_key() -> str:
    ...
    In production, keys are provisioned via the PACT dashboard.
    """
...
def run_demo() -> None:
    ...
    """
    print("=" * 72)
    print("  Tailor <> PACT Legislation API Integration Demo")
    print("=" * 72)
    print()
    ...
    print(f"    {'Metric':<30} {'ClauseTreeExtractor':>22} {'PACT API':>22}")
    print(f"    {'─' * 30} {'─' * 22} {'─' * 22}")
    ...
    print("=" * 72)
    print()
    print("  PACT provides pre-parsed, canonically-structured legislation")
