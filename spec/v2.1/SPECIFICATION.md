# PACT — Protocol for Agent Contexture and Trust — Specification v2.1.0

> **Status:** DRAFT — NOT FOR CITATION. Pending maintainer sign-off per AGENTS.md rule 3. See [`README.md`](./README.md).  
> **Author:** Knox Hart + AI  
> **Date:** Draft — v2.1 line  
> **Version:** 2.1.0 (first minor after the v2.0 line; adds **§19 Parleys**, **§20 the Mandate primitive**, **§21 push delivery**, and **§22 service-account authentication** to a v2.0.3 carry-forward. Additive — no breaking changes to v2.0.x clients.)  
> **Carried forward from:** v2.0.3, unchanged except where a section below is marked v2.1. `spec/v2.0/` remains FROZEN per AGENTS.md rule 4.  
> **Version note:** 2.0.3 (third patch on the v2.0 line; adds **fabric onboarding & session awareness** — the "cognitive layer" of active session: agents can now atomically join+constrain a fabric, query their scoped manifest of obligations, heartbeat bidirectionally, and acknowledge event ranges. Additive — no breaking changes to v2.0 / v2.0.1 / v2.0.2 clients. See [`CHANGELOG.md`](../../CHANGELOG.md#v203--2026-05-15) for the operations added.)  
...
PACT v2.1 adds the protocol's **second interaction mode**. Through v2.0.3 every PACT interaction was a long-lived shared resource with a fabric (§4.4) around it. v2.1 adds the bounded, bilateral shape — "have my agent talk to that agent for five minutes about this one thing" — and the two primitives that make it operationally usable.

- **§19 Parleys** — ephemeral, time-bounded, small-N (2–5) negotiations between agents. Open → accept → negotiate → close with an outcome of `aligned`, `deadlocked`, `timeout`, or `mandate_revoked`. Lands RFC [#14](https://github.com/TailorAU/pact/issues/14).
- **§20 The Mandate primitive** — a handler-signed capability grant that pins what an agent may publish, must respect, may commit to, and may disclose. A **§17-family object**: same trust chain, same 1:1 HumanPrincipal model, same algorithm whitelist, same fail-closed verification. The server enforces it on every operation.
- **§21 Push delivery** — signed event webhooks. Serves both the operations-alerting case (#13 Q6) and the Mandate `escalation_hook`, in one envelope format.
- **§22 Service-account authentication** — the BYOK scoped-key model promoted from a side effect of the join-token flow to a first-class auth mode with create / rotate / revoke / narrow lifecycle. Explicitly **not** a substitute for proof of human intent.

**Composition, not duplication.** §19 reuses the v2.0.3 machinery rather than restating it: Parley `state` reuses the §4.4.2 caller-scoped manifest shape and §17.13 disclosure rules; round expectations are §6.5 pending obligations (`kind: "respond"`); liveness is §4.4.3 heartbeat; agent identity is §23 `agentId`; the Mandate is a §17-family object. The genuinely new surface is bounded to the Mandate, the ephemeral lifecycle, and the bilateral handshake.

**Resolves from issue [#13](https://github.com/TailorAU/pact/issues/13):** Q1 (self-approval — a Parley has no approval step, so cross-agent self-approval cannot arise), Q2 (§22), Q6 and Q7 (the Mandate `escalation_hook` is a structural binding back to the dispatching handler).

**The backronym is refined in v2.1.** The acronym **PACT is unchanged**; the expansion is refined from "Protocol for Agent **Consensus and Truth**" to "Protocol for Agent **Contexture and Trust**":

> **PACT — Protocol for Agent Contexture and Trust:** the shared resource and the context that gives it meaning travel together — so agents negotiate with trust.

"Trust" replaces "Truth" because the protocol establishes trust and consensus over a shared context; it does not adjudicate truth. That matches what actually shipped — the §17 trust model, the fail-closed conformance posture, the §17.13 honesty about what PACT cannot guarantee. "Contexture" names the core unlock: the shared resource and the context that gives it meaning travel together.

This is a **backronym refinement, not a rename**. No identifier changes: `@pact-protocol/*` package names, the `pact-spec.dev` schema `$id` base, the repository, and the v2.0.x git tags are all unchanged. The new expansion is **normative from v2.1**; **shipped v2.0.x stays "Consensus and Truth" as-released**, and frozen versions (`spec/v0.3/` … `spec/v2.0/`) are NOT re-backronymed — they stay as-shipped for citation stability per AGENTS.md rule 4. "Contexture" is used only as the C-word in the expansion and as a concept term, never as a product, package, or domain name.
...
Tracked for v2.0 (normative text lands via coordinated PRs — see [`docs/v2-plan.yaml`](../../docs/v2-plan.yaml)): W3C DID principal identity (`did:web` + `did:key` required), an `Authorization-Required` conformance tier, agent identity lifecycle (§23), and a conformance test suite. The remaining v2.0 forward-references — ephemeral negotiation with handler-signed Mandates (§19–20, now **Parleys**), push delivery (§21), and service-account authentication (§22) — **land in this v2.1 directory**; see "What's New in v2.1" above.
...
// v2.1 — Parleys, Mandates (§19, §20)
pact.parley.opened              // Parley created; the opener's Mandate verified (§19.5)
pact.parley.joined              // A participant accepted with a verified Mandate (§19.5)
pact.parley.mandate-violation   // An operation exceeded a Mandate envelope, or envelopes conflict at open (§19.6, §19.7)
pact.parley.closed              // Parley terminal; payloadJson carries the parley-outcome.json shape (§19.8)
```
...
// Parleys & Mandates (v2.1 — §19, §20). Note: NOT under {documentId} — a Parley is not a resource.
POST   /api/pact/parleys                               // Open a Parley (opener's Mandate in body)
POST   /api/pact/parleys/{parleyId}/accept             // Accept with a Mandate; returns a join token
GET    /api/pact/parleys/{parleyId}/state              // Caller-scoped state (§4.4.2 manifest shape)
POST   /api/pact/parleys/{parleyId}/positions          // Post a negotiating position (envelope-checked)
POST   /api/pact/parleys/{parleyId}/_heartbeat         // Liveness (§4.4.3 semantics)
POST   /api/pact/parleys/{parleyId}/close              // Propose a terminal outcome
DELETE /api/pact/parleys/{parleyId}/mandates/{mandateId} // Revoke a Mandate — immediate hang-up (§20.6)

// Push delivery (v2.1 — §21)
POST   /api/pact/{resourceId}/subscriptions            // Create a push subscription
...
GET    /api/pact/{resourceId}/subscriptions/{id}/dead-letters  // Retrieve dead-lettered deliveries

// Service accounts (v2.1 — §22)
POST   /api/pact/service-accounts                      // Create; credential returned ONCE
GET    /api/pact/service-accounts                      // List (metadata only, never the credential)
POST   /api/pact/service-accounts/{id}/rotate          // Rotate the credential (overlap window)
DELETE /api/pact/service-accounts/{id}                 // Revoke immediately
...
| `capabilities` | SHOULD | Boolean capability flags. v2.0 well-known flags: `mediatedCommunication`, `informationBarriers`, `structuredNegotiation`, `inviteTokens`, `authorizationProof`, `agentIdentityTransfer`, `didDocumentPinning`, `recoverySingleChannel`, `atomicOnboard` (v2.0.3), `manifest` (v2.0.3), `sessionAwareness` (v2.0.3). v2.1 adds `parleys` (§19.11), `parleyBindingOutcomes` (§19.8 — MUST NOT be claimed without §17 proof verification), `pushDelivery` (§21.5), and `serviceAccounts` (§22.4). *Note:* the v2.0 profile text reserved a `sessions` flag for this primitive; SOQ1 resolved the noun to "Parley", so the v2.1 flag is **`parleys`** and `sessions` is not used. |
...
| **Extended** | Core + information barriers (classification, clearance, graduated disclosure), mediated communication, structured negotiation, invite tokens. **v2.1:** SHOULD support Parleys (§19), push delivery (§21), and service-account auth (§22) | Enterprise, regulated, multi-organisation |
...
**v2.1 note.** Parleys, push delivery, and service accounts are OPTIONAL at Core and SHOULD be supported at Extended. Each is advertised by its own §15.1 capability flag, and advertising a capability carries its enforcement obligations: `parleys` requires §20.7 Mandate-envelope enforcement, and at the `Authorization-Required` tier every Mandate MUST carry a valid handler signature (§20.5). A capability advertised without its enforcement is a false conformance claim, not a partial implementation.
...
Any PACT message (proposal, intent, constraint, completion, mediated message, Mandate) MAY include an `authorization_proof` object:

> **Terminology clarification (v2.1).** Earlier text in this subsection referred to a "session mandate". From v2.1 that phrase binds to **§20 — the Mandate primitive** carried by a **§19 Parley**, *not* to the §4.4 fabric session-awareness layer. See the §19.2 terminology note. v2.0's wording is frozen and is not amended retroactively.
...
> **Status:** v2.1 normative. Lands RFC [#14](https://github.com/TailorAU/pact/issues/14) (verdict ACCEPT-WITH-MODIFICATIONS, comment window closed 2026-05-26) under decisions D3=A and D5=A. Design record: `docs/v2-plan.yaml` track T3, `docs/v2-prep/rfc-sessions-mandate.md`, `docs/v2-prep/rfc-14-shepherd-synthesis.yaml`, `docs/v2-prep/v2.1-scope.yaml`. Resolves issue [#13](https://github.com/TailorAU/pact/issues/13) Q1 (self-approval), Q6 and Q7 (escalation routing).

### 19.1 Problem

Through v2.0.3, PACT modelled exactly one interaction shape: a long-lived shared **resource** (§3) whose coordination context is a **fabric** (§4.4). Agents join the fabric, declare intents and constraints, propose changes, and consensus emerges over time. That model is correct for collaborative authoring and heavyweight for a bounded question between two agents.

Three gaps surface from production use:

1. **Self-approval (#13 Q1).** When two agents operate under one HumanPrincipal, the §5 approval policy rejects any cross-agent approval. AloomU hit this in their inaugural deployment. The long-lived model has no shape in which the question does not arise.
2. **Escalation routing (#13 Q6 / Q7).** Agents escalate to "whoever is watching the resource," not back to the specific handler that dispatched them. There is no protocol-level binding between an agent and the human who sent it.
3. **The bilateral shape.** Many real engagements are "have my agent talk to that agent for five minutes about this one thing," not "join a persistent fabric." v2.0.3 has no shape for that.

§19 adds PACT's **second interaction mode** to close all three. It complements the resource model; it does not replace it.

### 19.2 Concept and terminology

A **Parley** is an ephemeral, time-bounded, small-N negotiation between agents. It opens, runs to a verdict, and closes. Each agent enters under a **Mandate** (§20) — a handler-signed capability grant that pins what the agent may publish, must respect, may commit to, and may disclose. The server enforces the Mandate on every operation and reports the outcome to each handler's registered hook (§21).

**Terminology note (normative).** The primitive is a **Parley** (capitalised for the primitive; "a Parley", plural "Parleys"). This is the same disambiguation pattern v2.0.3 §4.4 used when it minted "fabric". A Parley is explicitly **not** any of the following, and the bare unqualified word "Session" MUST NOT be used to denote a Parley:

| Not to be confused with | What that is | Where |
|---|---|---|
| **Session manifest / session awareness** | The awareness layer over a *long-lived fabric* — `_status`, `manifest`, `_heartbeat`, `mark-read`, `_onboard`. Shipped in v2.0.3 and unchanged here. | §4.4 |
| **Mediated negotiation rounds** | The Mediator-driven multi-round exchange on a contested section (`negotiation.open`, the §13.6 Mediator API). An existing, distinct concept. | §13 |
| **A Parley** | The ephemeral, caller-initiated, handler-mandated negotiation defined in this section. | §19 |

**Binding the §17.6 reference.** §17.6 lists "session mandate" among the messages that MAY carry an `authorization_proof` envelope. In v2.1 and later that phrase binds to **§19 — the Parley** and its §20 Mandate, **not** to the §4.4 fabric session-awareness layer. v2.0's §17.6 text is frozen (it shipped before this section existed) and is not amended retroactively; readers of v2.0 should read the phrase as anticipatory.

A Parley is also **not**:

- **A replacement for fabrics.** Long-lived collaboration is still a resource (§3–§12). A Parley that needs to outlive its TTL is the wrong shape; use a resource.
- **A contract system.** Outcomes are advisory by default. Binding outcomes exist, are opt-in, and are cryptographically witnessed — see §19.8.
- **A federation primitive.** Federation remains a v2.1 non-goal. §19–§21 endpoints MUST NOT preclude it: Parley identifiers are portable and transcripts are canonical JSON (RFC 8785). (The v2.2 Matter primitive states the same constraint for its own endpoints; v2.2 is a parallel carry-forward and its §24 is not part of this document.)

### 19.3 Composition with v2.0.3 — reuse, do not duplicate

v2.0.3 shipped the awareness and atomic-onboard machinery for the long-lived mode. The Parley mode **reuses** it and adds only what is genuinely new: the Mandate primitive, the ephemeral lifecycle, and the bilateral handshake. The following reuse points are normative — an implementation MUST NOT introduce a parallel mechanism where this table names an existing one.

| Concern | Parley mode uses | Not |
|---|---|---|
| Authorization envelope | The Mandate (§20) is a **§17-family object**: same trust chain (§17.2), same 1:1 HumanPrincipal model (§17.4), same `alg` whitelist discipline (§17.6), same fail-closed verification posture (§17.7). | A parallel signature or identity model. |
| Agent identity | §23 `agentId` — server-side, persistent, federation-portable. | A Parley-local agent handle. |
| "Where am I" | `GET /api/pact/parleys/{parleyId}/state` reuses the §4.4.2 caller-scoped manifest **shape** and the §17.13 disclosure rules, scoped to the Mandate envelope instead of resource constraints. | A parallel manifest format. |
| Round expectations | §6.5 pending obligations with `kind: "respond"`. | A Parley-specific "turn owed" mechanism. |
| Liveness | §4.4.3 heartbeat semantics and `attention_required`. A silent peer driving a Parley to `timeout` is the same liveness signal §4.4.3 already defines. | A Parley-specific ping. |
| Outcome and violation delivery | §21 push delivery — the Mandate's `escalation_hook` is a per-Mandate subscription expressed in the §21 signed-delivery envelope. | An ad-hoc webhook format. |
| Disclosure ceiling | §10.3 graduated-disclosure levels 1–4. | A new disclosure scale. |

The `_onboard` operation (§4.4.5) is **not** reused: a Parley's open/accept handshake is the Parley-mode analogue of atomic onboarding, not a second caller of it. §19.5 defines that handshake as its own atomic operation, and it MUST NOT be specified in terms of `_onboard`.

### 19.4 Lifecycle

```
                    ┌──────────── mandate revoked (§20.6) ────────────┐
                    │                                                 ▼
  opening ──→ awaiting-peer ──→ active ──→ closing ──→ closed { aligned
     │              │              │                            | deadlocked
     │              │              │                            | timeout
     │              │              │                            | mandate_revoked }
     │              │              └─ pact.parley.mandate-violation (does not close)
     │              └─ pact.parley.joined
     └─ pact.parley.opened
```
...
|---|---|---|
| `opening` | The opener's Mandate is being verified. No peer is aware of the Parley yet. | `POST /api/pact/parleys` |
| `awaiting-peer` | Opener's Mandate verified; the Parley is waiting for the peer to accept. The out-of-band handshake (§19.5) happens in this phase. | Successful Mandate verification |
| `active` | All expected participants have accepted with verified Mandates. Negotiation proceeds. | Final `POST …/accept` |
| `closing` | A terminal condition was reached; outcome is being assembled and pushed. | Any terminal trigger |
| `closed` | Terminal. The Parley is immutable; its transcript is retained per §6.3. | Outcome delivered or dead-lettered |

**Outcomes.** A closed Parley carries exactly one `outcome`:

| Outcome | Meaning |
|---|---|
| `aligned` | Participants converged. The `outcome_summary` records what was agreed. |
| `deadlocked` | Participants could not converge, or their Mandate envelopes were irreconcilable at open (§19.6). |
| `timeout` | The Parley's TTL expired, or a participant went silent past the liveness bound (§4.4.3). |
| `mandate_revoked` | A participant's Mandate was revoked mid-Parley (§20.6). |

**Small-N.** A Parley is designed for **2–5 participants**. Implementations MUST reject `POST /api/pact/parleys` with more than 5 expected participants (`parley.too_many_participants`, 400). Larger negotiations use a long-lived resource.

**TTL.** Every Parley has a TTL. The effective TTL is the **minimum** of the requested `ttl_seconds` and the earliest `expires_at` across all accepted Mandates — a Parley MUST NOT outlive the narrowest authority operating inside it. Implementations SHOULD cap `ttl_seconds` and MUST document the cap in their §15.1 Implementation Profile.

### 19.5 Opening and the bilateral handshake

`POST /api/pact/parleys` opens a Parley. The caller is the **opener**; the caller's handler supplies the opener's Mandate.

**Request** (matches `parley-create-request.json`):
...
| Field | Required | Description |
|---|---|---|
| `purpose` | Yes | Human-readable statement of what the Parley is about. Carried into the outcome and into every participant's `state`. |
| `ttl_seconds` | Yes | Requested lifetime from `active`. Effective TTL per §19.4. |
| `expected_participants` | No (default `2`) | Total participants including the opener. 2–5. |
| `mandate` | Yes | The opener's Mandate (§20). Verified before the Parley leaves `opening`. |
| `outcome_binding` | No (default `"advisory"`) | `advisory` or `commitment`. See §19.8 — `commitment` imposes additional requirements at close. |
| `predecessor_parley_id` | No | Optional link to a prior Parley (§19.9). |

**Response** (matches `parley-create-response.json`):

```json
{
  "parley_id": "par_7bd2",
  "phase": "awaiting-peer",
  "peer_invite_url": "https://api.example/api/pact/parleys/par_7bd2/accept?invite=…",
  "expires_at": "2026-05-12T18:15:00Z",
  "event_id": "evt_9001"
}
```

**The handshake is out-of-band.** The opener transmits `peer_invite_url` to the peer's handler by a channel PACT does not specify (the same posture §15.6 takes for fabric invite tokens). PACT does not define handler discovery; a Parley cannot be opened *at* an agent that has not been invited by a human on the other side. This is deliberate — it is what makes the handler the root of authority rather than the server.

`POST /api/pact/parleys/{parleyId}/accept` joins the Parley. The body carries the accepting participant's Mandate (matches `parley-accept-request.json`); the response (`parley-accept-response.json`) returns `parley_id`, the assigned `agent_id` (§23), a `join_token` the handler hands to its agent, and the resulting `phase`.

**Atomicity.** Open and accept are each atomic in the §4.4.5 sense: either the participant is in the Parley *with a verified Mandate*, or it is not in at all. A Mandate that fails §20.5 verification MUST leave no partial participation and MUST NOT emit `pact.parley.joined`. There is no "joined but unmandated" state.

**Errors.** `auth.unauthorized` (401), `mandate.invalid` (403 — see §20.5 for the failing-step detail), `mandate.expired` (403), `parley.not_found` (404), `parley.already_active` (409 — accept arrived after the expected participant count was met), `parley.closed` (409), `parley.too_many_participants` (400), `rate.limited` (429).

**Events.** `pact.parley.opened` on successful open; `pact.parley.joined` per successful accept.

### 19.6 Mandate intersection at open (OQ2)

Two handlers may issue Mandates whose envelopes cannot both be satisfied — Handler A requires a price decision, Handler B forbids price commitments. On accept, the server computes whether the accepting Mandate's `constraint_envelope` is reconcilable with those already accepted.

**The Parley opens anyway.** An irreconcilable intersection MUST NOT cause the accept to fail. The Parley transitions to `active` with a machine-readable `intersection_conflicts` array attached to its state, and the server MUST emit `pact.parley.mandate-violation` with `payloadJson.kind = "intersection_conflict"` naming the conflicting boundaries.

Refusing to open would hide the disagreement; opening with the conflict surfaced lets the agents renegotiate scope or escalate to their handlers — consistent with PACT's event-sourced, make-the-state-visible principle (§2). A Parley whose conflicts are never resolved closes `deadlocked`, and `outcome_summary.intersection_conflicts` records why.

### 19.7 Negotiating inside the envelope

Within an `active` Parley, participants exchange positions. Each operation is checked against the acting participant's Mandate by the §20.7 enforcement algorithm **before** it takes effect.

| Operation | Method + Path | Notes |
|---|---|---|
| Post a position | `POST /api/pact/parleys/{parleyId}/positions` | Body is a §13.5 negotiation position. Checked against `constraint_envelope.may_publish` and `disclosure_ceiling`. |
| Read state | `GET /api/pact/parleys/{parleyId}/state` | Caller-scoped; §4.4.2 manifest shape, §17.13 disclosure rules. |
| Heartbeat | `POST /api/pact/parleys/{parleyId}/_heartbeat` | §4.4.3 semantics verbatim. |
| Close | `POST /api/pact/parleys/{parleyId}/close` | Proposes a terminal outcome. See §19.8. |

**Round expectations are §6.5 obligations.** When a participant posts a position that expects a reply, the server registers a §6.5 pending obligation with `kind: "respond"` against each other participant, `event_ref` pointing at the position event. The obligation discharges when that participant posts its own position, and is `superseded` if the Parley closes first. The `kind` enum of §6.5 is **unchanged** — `respond` already covers this; §19 does not shadow the obligation model with a Parley-local one.

**Liveness.** Participants heartbeat per §4.4.3. A participant that misses the liveness bound the implementation documents in its Implementation Profile drives the Parley to `timeout`. `attention_required: true` has the same meaning it has on a fabric.

**Mandate violations do not close the Parley.** An operation that would exceed the acting participant's envelope is **rejected** (`mandate.violation`, 403) and emits `pact.parley.mandate-violation`. The Parley continues. Violations are pushed to the violating participant's handler via `escalation_hook` (§21) so the handler — not the server — decides whether to revoke, widen the Mandate, or let the agent retry within bounds. A server MUST NOT auto-close a Parley on violation; that is a handler decision.

### 19.8 Outcomes and binding (OQ3, tightened per SOQ4)

`POST /api/pact/parleys/{parleyId}/close` proposes a terminal outcome. The Parley closes when every participant has posted a matching close, when the TTL expires, or when a Mandate is revoked.

**Advisory by default.** A Parley closed with `outcome_binding: "advisory"` (the default) produces an outcome that is a **report to the handlers**. It binds nobody. The handler decides what, if anything, to do with it. Outcomes MUST NOT automatically create downstream resources — that is an explicit non-goal.

**Binding outcomes MUST be cryptographically witnessed.** Where `outcome_binding: "commitment"`, the close is valid only if **every** participant's close carries a per-handler §17.6 `authorization_proof` over the outcome body, verified per the §17.7 flow. A `commitment_authority` grant in the Mandate (§20.3) is **necessary but not sufficient**: the Mandate says the handler *permits* a binding commitment of this shape; the `authorization_proof` on the close is the handler *making* it, at the moment it is made, over the specific text.

A close claiming `commitment` without a valid per-handler proof from every participant MUST be rejected (`outcome.unwitnessed`, 403) and MUST NOT close the Parley. An implementation that cannot verify §17 proofs MUST NOT advertise `outcome_binding: "commitment"` support and MUST reject `commitment` closes — fail closed, never open.

> This is the line between a Parley and contracts reinvented over HTTP. A flag in a request body is an API assertion; a per-handler signature over the outcome text is a commitment. PACT requires the second.

**Outcome shape** (matches `parley-outcome.json`):
...
```json
{
  ...
  "outcome_summary": {
    ...
    "positions_ref": ["evt_9042", "evt_9051"],
    "intersection_conflicts": []
  },
  "participants": [
    { "agent_id": "urn:pact:agent:k-1", "handler_principal_id": "did:web:knox.example", "mandate_id": "mnd_a1" }
  ],
  "opened_at": "2026-05-12T18:00:00Z",
  "closed_at": "2026-05-12T18:11:42Z",
  "witness_proofs": [],
  "transcript_hash": "base64url-…"
}
```

`transcript_hash` is the SHA-256 of the canonical JSON (RFC 8785) encoding of the Parley's event sequence, so an outcome delivered to a handler can be checked against the transcript it claims to summarise. `witness_proofs` carries the per-handler §17.6 proofs and MUST be non-empty when `outcome_binding` is `commitment`.

**Delivery.** On close, the server pushes the outcome to every participant's Mandate `escalation_hook` using the §21 signed-delivery envelope. Delivery follows §21.4 semantics — at-least-once, retried with backoff, dead-lettered after 24 hours. A Parley is `closed` regardless of whether delivery succeeds; delivery failure is reported through the §21 dead-letter surface, and MUST NOT reopen the Parley.

**Event.** `pact.parley.closed`, carrying the full outcome shape in `payloadJson`.

### 19.9 Carry-forward (OQ4)

Parleys are **islanded by default**: a Parley carries no state from any predecessor, and participants in one Parley learn nothing about another.

`predecessor_parley_id` optionally links round *n* to round *n−1*. When present:

- The link is **metadata only**. No positions, transcript, or Mandate content carries forward. Each Parley is separately mandated.
- The **existence of the predecessor is itself information** and is subject to §17.13 disclosure. A participant sees `predecessor_parley_id` in `state` only if it was a participant in that predecessor, or if every participant of the predecessor is also a participant here. Otherwise the field is **omitted** (key absent), not nulled — the §17.13 omission rule.
- Implementations MUST NOT infer authority from linkage. A Mandate that expired in the predecessor is expired, full stop.

### 19.10 Events

```
pact.parley.opened              // Parley created; opener's Mandate verified (§19.5)
pact.parley.joined              // A participant accepted with a verified Mandate (§19.5)
pact.parley.mandate-violation   // An operation exceeded a Mandate envelope, or envelopes conflict at open (§19.6, §19.7)
pact.parley.closed              // Terminal; payloadJson carries the parley-outcome.json shape (§19.8)
```

These follow the §6.1 event structure. `pact.parley.opened` and `pact.parley.closed` have `actorKind` of the opener and `System` respectively; `pact.parley.mandate-violation` is `System`. `correlationId` on `joined`, `mandate-violation`, and `closed` MUST point at the `pact.parley.opened` event so a consumer can reconstruct one Parley as a single chain.

### 19.11 Conformance

| Level | Requirement |
|---|---|
| **Core** | Parleys are **OPTIONAL**. A Core implementation MAY be resource-only; this is unchanged from v2.0. |
| **Extended** | **SHOULD** support Parleys. If `capabilities.parleys` is advertised in the §15.1 Implementation Profile, the implementation **MUST** enforce Mandate envelopes per §20.7 — advertising Parleys without envelope enforcement is a false conformance claim. |
| **Authorization-Required** | **MUST** require a valid handler signature on every Mandate. There are no anonymous cross-organisation Parleys. This is the §17.9 cross-org rule applied at Mandate scope. |

An implementation supporting Parleys advertises `capabilities.parleys: true` (§15.1). Support for binding outcomes (§19.8) is advertised separately as `capabilities.parleyBindingOutcomes: true`, and MUST NOT be claimed by an implementation that cannot verify §17 proofs.

Conformance vectors live in `spec/v2.1/conformance/extended/parleys/`. The Mandate-enforcement vectors are **fail-closed**: an implementation whose Mandate verifier is unimplemented MUST fail those vectors, never pass them by default. This follows the posture of the §18.3 voice-biometric contract — an unimplemented verifier is `unverifiable`, never `verified`.

---

## 20. The Mandate Primitive (v2.1)

> **Status:** v2.1 normative. The Mandate is the object §17.6 anticipated when it listed "session mandate" among envelope-bearing messages. Design record: `docs/v2-plan.yaml` track T3; RFC [#14](https://github.com/TailorAU/pact/issues/14) decision D3=A.

### 20.1 What a Mandate is

An `authorization_proof` (§17.6) attests **"this human authorized *this message*."** A **Mandate** attests **"this human authorized *this agent* to operate within *these bounds* for *this Parley*."** Same trust chain, broader temporal scope.

A Mandate is a **§17-family object**. This is the single most important composition rule in v2.1:

- It is signed by a **HumanPrincipal** under the §17.4 strictly-1:1 model.
- Its `alg` is drawn from the §17.6 normative whitelist, and anything outside that whitelist MUST be rejected.
- It is verified by the §17.7 flow, applied to a broader-scope claim (§20.5).
- Verification is **fail-closed** (§17.7 step 6): any failure means the Mandate is not valid, and an agent without a valid Mandate does not enter the Parley.

A Mandate is **not** a bearer token. It is not sufficient for transport authentication; §22 covers that. It authorises a *negotiating posture*, not a session cookie.
...
```json
{
  ...
  "constraint_envelope": {
    ...
    "must_respect": [
      ...
      { "boundary": "p99 latency under 200ms" }
    ]
  },
...
| Field | Type | Required | Description |
|---|---|---|---|
| `version` | string | Yes | Mandate format version. `"1"` at v2.1. |
| `mandate_id` | string | Yes | Handler-minted identifier, unique per handler. Named in violation events and in the outcome's `participants[]`. |
| `parley_id` | string | Conditional | The Parley this Mandate authorises. REQUIRED on `accept`. On `open` the Parley does not yet exist, so it is absent and the server binds the Mandate to the Parley it mints (§20.5 step 7). A Mandate bearing a `parley_id` MUST NOT be accepted for any other Parley. |
| `agent_id` | string | Yes | The §23 `agentId` this Mandate authorises. A Mandate is not transferable between agents. |
| `handler_principal_id` | string (DID) | Yes | The HumanPrincipal granting the Mandate. §17.4 1:1 model. |
| `identity_claim` | string | Yes | Human-readable assertion of what the agent represents. Surfaced verbatim to counterparties in `state` — this is what the other side is told it is talking to. |
| `constraint_envelope.may_publish` | array of string | Yes | Topic categories the agent may publish positions in. An operation outside these categories is a violation (§19.7). May be empty — an agent that may say nothing is a listener. |
| `constraint_envelope.must_respect` | array of object | Yes | Boundaries the agent must not cross, each `{ "boundary": "<text>" }`. Boundaries are §10.3-style constraints: they state the bound, not the reasoning behind it. |
| `commitment_authority` | object | No | `{ max_binding_decisions: int, binding_scope: string }`. Caps irreversible decisions. Absent → the agent has no commitment authority and any `commitment` close is rejected. **Necessary but not sufficient** for a binding outcome — see §19.8. |
| `disclosure_ceiling` | integer 1–4 | Yes | Maximum §10.3 graduated-disclosure level the agent may reach. An operation disclosing above this level is a violation. |
| `escalation_hook` | string (URI) | Yes | Where outcomes and violation events are pushed, in the §21 signed-delivery envelope. MUST be `https`. |
| `expires_at` | ISO 8601 | Yes | Expiry **in server time** (§20.4). |
| `credential_id` | string | Yes | The enrolled credential that produced `signature`, resolvable under `handler_principal_id` per §17.8. |
| `signing_key_id` | string | Yes | Key identifier within the handler's DID Document. |
| `alg` / `alg_version` | string | Yes | Per §17.6. Outside the whitelist → reject at §20.5 step 3. |
| `signature` | string | Yes | Signature over the canonical JSON (RFC 8785) encoding of the Mandate body with `signature` removed. |

### 20.3 The envelope fields, in force

The four envelope fields are what the server actually enforces on every operation (§20.7):

- **`may_publish`** is a *permit list of topics*, not of content. The agent may publish more detail than the handler pre-loaded — it may not publish in a category the handler did not name.
- **`must_respect`** boundaries are checked against positions the agent posts. Boundary evaluation is necessarily partly semantic; an implementation MUST document its evaluation method in its Implementation Profile, and MUST fail closed where it cannot evaluate a boundary — an unevaluable boundary is a violation, not a pass.
- **`commitment_authority`** caps binding decisions. Exceeding `max_binding_decisions`, or committing outside `binding_scope`, is a violation and triggers escalation.
- **`disclosure_ceiling`** caps the §10.3 level. A ceiling of 2 means the agent may share boundaries and category tags, never full reasoning (level 3) or human-only material (level 4).

### 20.4 Expiry, clock, and skew

**The server's clock is authoritative.** `expires_at` is in server time. This resolves RFC #14 OQ6.

The permitted skew between a Mandate's asserted time base and the server's clock is the **§17.7 step 4 window — ±5 minutes, configurable** — reused verbatim. A Parley-specific bound is deliberately **not** introduced: a tighter threshold for a Mandate than for a single-message `authorization_proof` would be an inconsistency inside one trust model, and the Mandate is the *broader* grant of the two.

A Mandate whose `expires_at` has passed is invalid. An expired Mandate mid-Parley behaves as revocation (§20.6): the Parley closes `mandate_revoked`. Implementations MUST evaluate expiry against the effective TTL rule in §19.4 — a Parley never outlives the narrowest Mandate inside it.

### 20.5 Verification at open and accept

Mandate verification is the §17.7 flow applied to a broader-scope claim. On receiving a Mandate the server MUST, in order:

1. **Type dispatch** — resolve the signature suite for `alg` (§18). Unrecognised → `unverifiable` → reject.
2. **Principal resolution** — resolve `handler_principal_id` per §17.8 / DID resolution. Failure → reject.
3. **Algorithm whitelist** — `alg` MUST be in the §17.6 whitelist. Outside it → reject. HMAC and symmetric-key algorithms are disallowed, as in §17.6.
4. **Signature verification** — verify `signature` against the key enrolled for `credential_id` under `handler_principal_id`, over the canonical JSON encoding of the Mandate body less `signature`.
5. **Freshness and expiry** — `expires_at` MUST be in the future in server time, within the §20.4 skew window.
6. **Revocation** — the credential MUST NOT be revoked or tombstoned in the §17.8 registry; the Mandate's `mandate_id` MUST NOT appear in the handler's revocation surface (§20.6).
7. **Binding** — `agent_id` MUST match the joining agent. On `accept`, `parley_id` MUST equal the Parley being joined. On `open`, the server binds the minted `parley_id` to the Mandate and records the binding in the `pact.parley.opened` event.
8. **Result** — any failure → reject the open/accept with `mandate.invalid` (403), naming the failing step, and MAY emit `pact.trust.violation` with `payloadJson.kind = "mandate_failed"`. Success → the agent enters the Parley under this Mandate.

**Fail closed.** An implementation that cannot perform a step MUST treat the Mandate as invalid. There is no "accepted pending verification" state, and no default-open path.

**Cross-organisation.** At the `Authorization-Required` tier, every Mandate MUST carry a valid handler signature — the §17.9 rule at Mandate scope. A Parley across an organisational boundary with an unsigned or unverifiable Mandate MUST NOT open.

### 20.6 Revocation (OQ1)

A handler revokes a Mandate by publishing the revocation on the surface it already uses for credential state (§17.8) or by calling `DELETE /api/pact/parleys/{parleyId}/mandates/{mandateId}` where the implementation supports it.

**Revocation is an immediate hang-up.** On learning of a revocation for a Mandate in an `active` Parley, the server MUST:

1. Stop accepting operations from the revoked participant immediately — mid-round, not at the end of one.
2. Transition the Parley to `closing` and then `closed` with `outcome: "mandate_revoked"`.
3. Push the outcome to every participant's `escalation_hook` (§21).

Finishing the current round is explicitly **not** permitted: it would risk an agent committing under an authority that no longer exists, and a revoked authorization must never retroactively bless an in-flight commitment. This is the §17 trust model applied consistently — revocation is not advisory.

Operations already committed before the revocation was known remain in the transcript. Revocation is not retroactive over the event log; it is immediate over future operations. The outcome records `revoked_mandate_id` so a handler reading the transcript can see exactly where authority ended.

### 20.7 Enforcement reference algorithm

For every operation an agent attempts inside an `active` Parley, the server evaluates:

```
enforce(operation op, mandate m, parley p):
  1. if now() > m.expires_at            -> REVOKE_PATH (§20.6)
  2. if m.mandate_id is revoked         -> REVOKE_PATH (§20.6)
  3. if op.topic not in m.constraint_envelope.may_publish
                                        -> VIOLATION("may_publish", op.topic)
  4. if op.disclosure_level > m.disclosure_ceiling
                                        -> VIOLATION("disclosure_ceiling", op.disclosure_level)
  5. for each b in m.constraint_envelope.must_respect:
       verdict = evaluate(op, b)
       if verdict is CROSSED            -> VIOLATION("must_respect", b.boundary)
       if verdict is UNEVALUABLE        -> VIOLATION("must_respect", b.boundary)   // fail closed
  6. if op is a binding commitment:
       if m.commitment_authority absent -> VIOLATION("commitment_authority", "absent")
       if p.binding_decisions_by(m) + 1 > m.commitment_authority.max_binding_decisions
                                        -> VIOLATION("commitment_authority", "max_binding_decisions")
       if op.scope not within m.commitment_authority.binding_scope
                                        -> VIOLATION("commitment_authority", "binding_scope")
  7. ALLOW

VIOLATION(field, detail):
  - reject the operation with mandate.violation (403)
  - emit pact.parley.mandate-violation { mandate_id, field, detail }
  - push the violation to m.escalation_hook per §21
  - DO NOT close the Parley (§19.7) — the handler decides
```

Step 5's `UNEVALUABLE → VIOLATION` mapping is the fail-closed rule restated: an implementation that cannot decide whether a boundary was crossed MUST treat the operation as crossing it. An implementation that instead allows unevaluable boundaries is **not** conformant and MUST NOT advertise `capabilities.parleys`.

### 20.8 Delegation

Whether a Mandate may itself be delegated through a §17.11 `attestation_chain` is **out of scope for v2.1**. Mandates in v2.1 are **direct (1-hop) only**: `handler_principal_id` is the human who signed, with no intermediate attestations. A Mandate presented with a non-empty delegation chain MUST be rejected as `unverifiable`.

This matches §17.11, which defers delegation semantics generally. When delegation lands, Mandate participation in the chain is specified alongside it.

---

## 21. Push Delivery (v2.1)

> **Status:** v2.1 normative. Design record: `docs/v2-plan.yaml` track T4. Closes issue [#13](https://github.com/TailorAU/pact/issues/13) Q6. Ships together with §19–§20 because the Mandate `escalation_hook` is load-bearing for the Parley outcome path.

### 21.1 Problem

Through v2.0.3 every PACT consumer is a **poller**: `GET /events`, `GET /poll`, `GET /_status`. That is adequate for an agent in a negotiation loop and inadequate for the two cases production surfaced — an operations channel that must be alerted when something needs a human (#13 Q6: PagerDuty, Slack), and the Mandate `escalation_hook`, which must reach a handler that is not polling anything.

§21 defines one primitive that serves both: a registered endpoint receives a **signed event push** when a matching event fires.

### 21.2 Subscriptions

| Method | Path | Description |
|---|---|---|
| POST | `/api/pact/{resourceId}/subscriptions` | Create a subscription. |
| GET | `/api/pact/{resourceId}/subscriptions` | List the caller's subscriptions. |
| DELETE | `/api/pact/{resourceId}/subscriptions/{subscriptionId}` | Delete a subscription. |

**Request** (matches `subscription-create-request.json`):

```json
{
  "endpoint": "https://relay.example/hooks/pact",
  "event_types": ["pact.escalation.human", "pact.parley.closed"],
...
| Field | Required | Description |
|---|---|---|
| `endpoint` | Yes | Absolute `https` URL. Implementations MUST reject non-`https` endpoints and SHOULD reject endpoints resolving to private address ranges unless explicitly configured otherwise. |
| `event_types` | Yes | Event types (§6.2) to deliver. MUST be non-empty. A subscription MUST NOT be a wildcard for every event type an implementation might add later — subscribers name what they want. |
| `secret_hint` | No | Opaque key identifier the subscriber echoes back in its verification tooling. Not a secret itself. |
| `description` | No | Human-readable label. |

The response (matches `subscription-response.json`) returns `subscription_id`, the registered `endpoint`, `event_types`, `created_at`, and the server's `signing_key_id` so the subscriber knows which key to verify against.

**Scoping.** A subscription delivers only events the subscribing principal is entitled to observe. §17.13 disclosure rules apply to the delivered payload exactly as they apply to a read — push is not a disclosure bypass. Where a field would be elided in a `GET`, it MUST be elided in the pushed envelope.

### 21.3 The signed delivery envelope

Every push is a `POST` to the registered `endpoint` carrying (matches `event-delivery.json`):

```json
{
  "delivery_id": "dlv_3f19",
  "idempotency_key": "evt_9051:sub_77",
  "subscription_id": "sub_77",
  "event": { "...": "the §6.1 event structure" },
  "sent_at": "2026-05-12T18:11:43Z",
  "attempt": 1,
  "sender_principal_id": "did:web:api.example",
  "signing_key_id": "did:web:api.example#key-1",
  "alg": "webauthn-es256",
  "signature": "base64url-…"
}
```

**Signing.** `signature` covers the canonical JSON (RFC 8785) encoding of the envelope with `signature` removed, produced with the **sending server's principal key** (§17). `alg` MUST be in the §17.6 whitelist.

**Verification by the receiver.** A receiver MUST apply the §17.7 flow to the envelope: resolve `sender_principal_id`, check `alg` against the whitelist, verify `signature`, and check `sent_at` freshness within the §17.7 skew window. A receiver that cannot verify the signature MUST discard the delivery — an unverifiable push is not a push. Receivers MUST NOT act on the payload before verifying.

**Idempotency.** `idempotency_key` is stable across retries of the same event to the same subscription. A receiver MUST treat repeated deliveries bearing the same `idempotency_key` as one event. This is what makes at-least-once delivery safe.

### 21.4 Delivery semantics

- **At-least-once.** A receiver may see a delivery more than once; `idempotency_key` is how it deduplicates. Exactly-once is not offered and MUST NOT be assumed.
- **Success** is any 2xx response within the implementation's timeout.
- **Retry** on any non-2xx or timeout, with **exponential backoff and jitter**. Jitter is required, not optional — synchronised retries across many subscriptions are a self-inflicted outage.
- **Dead-letter** after **24 hours** of failed attempts. A dead-lettered delivery MUST be retrievable by the subscriber (`GET /api/pact/{resourceId}/subscriptions/{subscriptionId}/dead-letters`) and MUST NOT be silently dropped.
- **Ordering is not guaranteed.** Receivers order by the event's own sequence number (§6.1), never by arrival.

**The Mandate `escalation_hook` is a §21 delivery.** A Mandate's `escalation_hook` (§20.2) is a per-Mandate subscription, implicitly created at Parley open and deleted at close, delivering `pact.parley.closed` and `pact.parley.mandate-violation` for that Parley. It uses this envelope and these semantics — §20 defines *where* outcomes go; §21 defines *what the wire looks like*. There is no second webhook format.
...
|---|---|
| **Core** | OPTIONAL. |
| **Extended** | SHOULD support push delivery. |
| **Authorization-Required** | The delivery envelope MUST be signed with the sender's principal key (§17), and a receiver MUST apply the §17.7 flow to it. Unsigned push at this tier is non-conformant. |

Advertised as `capabilities.pushDelivery: true` (§15.1). Vectors live in `spec/v2.1/conformance/extended/push/`. **Federation note:** cross-server push is a v2.1 non-goal, but §21 endpoints MUST NOT preclude it — identifiers are portable and the envelope is self-describing, so a future federated sender fits this format unchanged.

---

## 22. Service-Account Authentication (v2.1)

> **Status:** v2.1 normative. Design record: `docs/v2-plan.yaml` track T5. Closes issue [#13](https://github.com/TailorAU/pact/issues/13) Q2.

### 22.1 Problem

v1.1's invite-token flow already returns a scoped `apiKey` suitable for a daemon. That key exists as a *side effect* of joining, with no lifecycle of its own — it cannot be rotated, narrowed, or revoked independently, and nothing names what it is allowed to do. Handlers that mint Mandates programmatically (§20) need exactly that: a long-lived, narrowly-scoped, rotatable credential.

§22 promotes the model to a first-class auth mode with an explicit lifecycle.

### 22.2 What a service account is — and is not

A service account is **transport identity**. It answers "which automated client is calling."

> **A service account is NOT a substitute for proof of human intent.** §22 MUST NOT be read as an authorization bypass. A service-account-authenticated message that crosses an organisational boundary still requires a valid §17.6 `authorization_proof` at the `Authorization-Required` tier, exactly as a bearer-authenticated one does. Authenticating the *daemon* says nothing about whether a *human* authorized the action. An implementation that accepts service-account authentication in place of an `authorization_proof` is non-conformant.

The same separation applies to Mandates: a service account may be scoped to *mint* Mandates, but the Mandate's `signature` is still the HumanPrincipal's (§20.5 step 4). A service account never signs a Mandate on a human's behalf.

### 22.3 Lifecycle

| Method | Path | Description |
|---|---|---|
| POST | `/api/pact/service-accounts` | Create a service account; returns the credential **once**. |
| POST | `/api/pact/service-accounts/{id}/rotate` | Issue a new credential; the previous one remains valid for the overlap window. |
| DELETE | `/api/pact/service-accounts/{id}` | Revoke immediately. |
| GET | `/api/pact/service-accounts` | List (metadata only — never the credential). |

**Create request** (matches `service-account-create-request.json`):
...
```json
{
  ...
  "scope": {
    ...
    "mandates": { "may_mint": true, "max_disclosure_ceiling": 2 }
  },
...
|---|---|
| `resources` | Resource / fabric IDs the account may act on. Empty or absent → **no** resource access, not "all". |
| `sections` | Section IDs within those resources. Absent → all sections of the named resources. |
| `operations` | Operations permitted: `read`, `propose`, `constrain`, `vote`, `close`, `subscribe`, `mint-mandate`. |
| `mandates` | The §20 seam. `may_mint` permits minting Mandates; `max_disclosure_ceiling` caps the `disclosure_ceiling` of any Mandate minted through this account, independent of what the handler requests. |

**Response** (matches `service-account-response.json`) returns `service_account_id`, `name`, `owner_principal_id`, `scope`, `expires_at`, and `credential` — the credential appears **only** in the create and rotate responses and MUST NOT be retrievable afterwards.

**Rotation.** `POST …/rotate` issues a new credential and keeps the previous one valid for an implementation-defined overlap window (SHOULD be at least 60 seconds, and MUST be documented in the Implementation Profile) so a running daemon can pick up the new credential without a failed request. Rotation MUST NOT widen scope.

**Scope narrowing.** An update MAY narrow a scope. It MUST NOT widen one — widening requires creating a new account, so an audit trail always shows the grant. An attempt to widen MUST be rejected (`scope.widening_forbidden`, 403).

**Revocation** takes effect immediately, including for in-flight requests where the implementation can enforce it.

### 22.4 Conformance

| Level | Requirement |
|---|---|
| **Core** | OPTIONAL. Implementations may continue with bearer or cookie auth. |
| **Extended** | SHOULD support service-account auth for daemon agents. |
| **Authorization-Required** | A service-account-authenticated cross-organisation message MUST still carry a valid §17.6 `authorization_proof` (§22.2). |

Advertised as `capabilities.serviceAccounts: true` (§15.1). Vectors live in `spec/v2.1/conformance/extended/service-accounts/`.

**Out of scope.** OAuth-based credential *enrollment* is §17.12 Q1 and remains deferred to a reviewed PR. §22 specifies the scoped-key model, not the enrollment story.
...
| `mandate.json` | Parley open / accept | Handler-signed capability grant — the Mandate primitive (v2.1 — §20) |
| `parley-create-request.json` | `POST /parleys` | Open a Parley (v2.1 — §19.5) |
| `parley-create-response.json` | `POST /parleys` | Parley id, phase, peer invite URL (v2.1 — §19.5) |
| `parley-accept-request.json` | `POST /parleys/{id}/accept` | Accept with a Mandate (v2.1 — §19.5) |
| `parley-accept-response.json` | `POST /parleys/{id}/accept` | Assigned agent id + join token (v2.1 — §19.5) |
| `parley-state.json` | `GET /parleys/{id}/state` | Caller-scoped Parley state, §4.4.2 manifest shape (v2.1 — §19.7) |
| `parley-outcome.json` | `POST /parleys/{id}/close`, `pact.parley.closed` | Terminal outcome + witness proofs + transcript hash (v2.1 — §19.8) |
| `subscription-create-request.json` | `POST /{resourceId}/subscriptions` | Push-delivery subscription request (v2.1 — §21.2) |
| `subscription-response.json` | `POST`/`GET` `/{resourceId}/subscriptions` | Subscription record (v2.1 — §21.2) |
| `event-delivery.json` | Push to a subscriber endpoint | Signed event-delivery envelope (v2.1 — §21.3) |
| `service-account-create-request.json` | `POST /service-accounts` | Service-account creation with scope (v2.1 — §22.3) |
| `service-account-response.json` | `POST /service-accounts`, `…/rotate` | Service-account record; `credential` present only on create/rotate (v2.1 — §22.3) |
...
*PACT Specification v2.1.0 — **DRAFT, not for citation**. A carry-forward of v2.0.3 (released 15 May 2026) adding §19 Parleys, §20 the Mandate primitive, §21 push delivery, and §22 service-account authentication. Promotion to stable requires explicit maintainer sign-off per AGENTS.md rule 3 — see [`README.md`](./README.md).*
