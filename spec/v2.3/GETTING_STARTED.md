...
> **What "silence" means here (§25.3, normative).** Absence of a protocol
> objection within the TTL window. Nothing more. It is **not** consent by a
> human, **not** an electronic signature, and **not** evidence that any human
> saw the proposal. An auto-merged document is an aligned **draft** — never
> `signed` or `executed` (§25.8).
>
> **And it does not apply to everything.** Objection-based auto-merge is only
> available where the apply is `internal-reversible` (§25.5). If applying the
> proposal would move money, leave your system, or purport to bind someone,
> the implementation MUST fail closed and wait for an explicit, payload-bound
> human attestation instead (§25.6). Configuring `objection-based` on such a
> resource does not override that — the guard wins.
