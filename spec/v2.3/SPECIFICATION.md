# PACT — Protocol for Agent Contexture and Trust — Specification v2.3 (DRAFT)

> **Status:** DRAFT — NOT FOR CITATION. This directory has not been signed
> off per AGENTS.md rule 3. Cite `spec/v2.2/` (stable, Matter-line) or
> `spec/v2.0/` (stable, v2.0.3) until this banner is removed by an explicit
> maintainer sign-off. Review vehicles:
...
> **Author:** Knox Hart + AI  
> **Date:** 20 August 2026  
> **Version:** 2.3.0-draft (carry-forward of the v2.2 body — itself v2.0.3
> plus §24 Matters — plus the **multi-cell fabric** rules: cells and the
> §15.4 cell trigger (#53), the `cell-quorum` approval policy and
> multi-cell policy bans (#54), and mandatory human escalation of
> multi-cell conflicts (#55). Additive for single-cell fabrics — no
> breaking changes for any fabric whose participant cell set has
> cardinality ≤ 1. Design record:
> `docs/v2-prep/rfc-multi-cell-quorum.md` Changes 1–4.)  
> **Carried v2.0.3 version note:** third patch on the v2.0 line; adds **fabric onboarding & session awareness** — the "cognitive layer" of active session: agents can now atomically join+constrain a fabric, query their scoped manifest of obligations, heartbeat bidirectionally, and acknowledge event ranges. Additive — no breaking changes to v2.0 / v2.0.1 / v2.0.2 clients. See [`CHANGELOG.md`](../../CHANGELOG.md#v203--2026-05-15) for the operations added.)  
...
- **Human approvals bucket by membership.** Humans are principals, not fabric memberships, and carry no `cell_id` of their own. A human approval counts toward the bucket of the **membership through which it is recorded**: the `cell_id` of the fabric membership (agent registration or human seat) that carries the approval into the fabric. A human principal whose agents hold memberships in more than one cell satisfies only the bucket of the membership actually used to approve. A human approval recorded through no membership at all (e.g. a host-side administrative action) does not satisfy any cell's bucket.
...
- **Enforcement binds to behaviour, not advertisement.** The rules in this section are unconditional on any fabric whose participant cell set has cardinality > 1. The `capabilities.cellQuorum` flag (§15.1) *advertises* support; it is not an opt-in switch, and not advertising it exempts nothing. A server that does not implement the multi-cell rules MUST NOT accept a multi-cell establish, a `cell_id`-bearing `join` / `_onboard`, or any membership that would make a fabric multi-cell — it rejects them (fail closed) rather than accepting the memberships and evaluating a banned policy over them.
...
- **`cell_id` MUST be proven, not merely asserted.** A membership's cell claim is a security input to every rule in this line — the fifth trigger above, §5 `cell-quorum` bucketing, and the dark-cell fail-closed rule — so it MUST be bound to a **proof of control of the claimed cell DID**, in one of two forms:
  - a **cell membership attestation**: a signature over at least `{fabric id, joining member's principal_id (or agentId), cell_id}` verifiable against a verification method of the cell DID, carried in the `join` / `_onboard` request; or
